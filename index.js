...
// function triggerGreeting(sessionId, carContext) {
//   const session = sessions.get(sessionId);
//   if (!session) return;
//   session.greetingTriggeredMs = Date.now();
//   console.log(
//     `[${sessionId}] Greeting triggered (${session.greetingTriggeredMs - session.startMs}ms)${
//       carContext ? ` | car: ${carContext}` : ""
//     }`,
//   );

//   // Inject a hidden system message so the AI knows which car the user is viewing
//   if (carContext) {
//     sendWsJson(session.ws, {
//       type: "conversation.item.create",
//       item: {
...
//           {
//             type: "input_text",
//             text: `[SYSTEM CONTEXT — do not read aloud] The customer opened this voice assistant from the ${carContext} page on the BYD Fairfield website. They are specifically interested in the ${carContext}. Greet them warmly, acknowledge you can see they're looking at the ${carContext}, and offer to help.`,
//           },
//         ],
//       },
//     });
...
//     const sessionId = socket.id;
//     // Accept carContext sent from the frontend (which car page the user opened)
//     const carContext = typeof data?.carContext === "string" && data.carContext.trim()
//       ? data.carContext.trim()
//       : null;
//     console.log(`[${sessionId}] Starting voice session${carContext ? ` | car: ${carContext}` : ""}`);
//     try {
//       let state = prewarmStates.get(sessionId);
//       if (!state) {
//         await startPrewarm(sessionId, forwarder);
...
//             clearPrewarmState(sessionId);
//             socket.emit("session-started", { sessionId });
//             triggerGreeting(sessionId, carContext);
//             return;
//           }
//         } catch {}
//         clearPrewarmState(sessionId);
